...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1, var=10):
        ...
        if not isinstance(style_image, np.ndarray) or \
           len(style_image.shape) != 3 or style_image.shape[-1] != 3:
            raise TypeError(
                "style_image must be a numpy.ndarray with shape (h, w, 3)")
        if not isinstance(content_image, np.ndarray) or \
           len(content_image.shape) != 3 or content_image.shape[-1] != 3:
            raise TypeError(
    ...
    def scale_image(image):
        """
        Scales an image such that its pixels values are between 0 and 1
        and its largest side is 512 pixels
        """
        if not isinstance(image, np.ndarray) or \
           len(image.shape) != 3 or image.shape[-1] != 3:
            raise TypeError(
    ...
    def style_cost(self, style_outputs):
        ...
        if type(style_outputs) is not list or len(style_outputs) != l:
            raise TypeError(
        ...
        cost = 0.0
        for i in range(l):
            cost += weight * self.layer_style_cost(style_outputs[i],
